# Игровая Механика и Правила

## Назначение
Этот документ описывает **правила и инварианты** игры Skazmor: игровой цикл, взаимодействия, ограничения и важные нюансы механики.
Он нужен как “спека механики”, а не как каталог контента.

## Источник правды (Source of Truth)
- Доменная механика:
  - `src/features/game/domain/reducer/gameReducer.ts` — основная логика правил (редьюсер).
  - `src/features/game/domain/deck/deckFactory.ts` — создание/перемешивание/генерация колоды (часть правил старта/раундов).
  - `src/features/game/domain/model/types.ts` — доменные типы (`GameState`, `Card`, ids и т.п.).
- Слой приложения (команды UI → GameAction):
  - `src/features/game/application/gameSession.ts`
- Контент (данные/описания):
  - `memory-bank/gameContent.md` + `src/content/*` (описания/иконки/цвета), **не** механика.

## Где в коде (Entry points)
- `src/features/game/domain/reducer/gameReducer.ts`
- `src/features/game/domain/deck/deckFactory.ts`
- `src/features/game/application/gameSession.ts`
- UI “где это проявляется”:
  - `src/components/game/screens/useGameScreenController.ts` (showInfo, проклятия, HUD-логика на уровне экрана)

Этот документ описывает основные игровые механики, правила взаимодействия объектов, ограничения и технические детали реализации игры Skazmor.

## 1. Основной Игровой Цикл

Игра построена на пошаговой системе управления ресурсами и картами.

1.  **Раздача (Deal):**
    *   На игровом столе есть **4 слота** для карт.
    *   В начале игры и при смене раунда слоты заполняются картами из колоды.
    *   Раунд продолжается до тех пор, пока на столе не останется **1 или 0 карт**.
    *   Когда это условие выполнено, начинается новый раунд: пустые слоты заполняются новыми картами из колоды.
        *   Если на столе **осталась 1 карта** → добираем **3**.
        *   Если на столе **0 карт** → добираем **4**.
2.  **Действия Игрока:**
    *   Игрок взаимодействует с картами на столе, перемещая их в руки или атакуя ими.
    *   Ход не ограничен по времени.
3.  **Условия Победы и Поражения:**
    *   **Победа:** Колода пуста **И** на столе не осталось монстров (все карты убраны).
    *   **Поражение:** Здоровье (HP) игрока падает до 0 или ниже.

## 2. Сущности и Ресурсы

### Игрок
*   **HP (Здоровье):** Стандартный максимум — 13. Может быть уменьшен способностью *Изнурение* или увеличен (теоретически, но обычно ограничен MaxHP).
*   **Coins (Монеты):** Валюта для покупки/продажи (в данной версии — счет очков).
*   **Руки (Slots):**
    *   **Left Hand / Right Hand:** Активные слоты для использования предметов (оружие, щиты, заклинания).
    *   **Backpack (Рюкзак):** Слот хранения. Предметы в рюкзаке нельзя использовать напрямую (кроме продажи), их нужно переложить в активную руку. Зелья в рюкзаке не пьются автоматически.

### Карты
Типы карт (`CardType`):
*   **Monster (Монстр):** Враг с показателем HP/Атаки.
*   **Weapon (Оружие):** Одноразовый предмет для атаки. Имеет значение урона.
*   **Shield (Щит):** Предмет для блокирования урона. Имеет значение прочности.
*   **Potion (Зелье):** Восстанавливает HP.
*   **Coin (Монета):** Дает золото.
*   **Spell (Заклинание):** Магический свиток с уникальным эффектом.
*   **Skull (Череп):** "Мусорная" карта, не имеет эффекта, занимает слот.

## 3. Механики Взаимодействия

### Проклятья (Curses)
**Проклятье** — это перманентный модификатор забега, который действует всю игру после активации.

**Правило выбора:**
- В начале забега проклятие можно **не выбирать**.
- Проклятие можно выбрать **только до первого действия** игрока.
  - Под “действием” понимаются игровые экшены: взять карту (`TAKE_CARD_TO_HAND`), взаимодействовать с монстром (`INTERACT_WITH_MONSTER`), применить заклинание (`USE_SPELL_ON_TARGET`), продать (`SELL_ITEM`), сбросить (`RESET_HAND`).
- После первого действия выбор блокируется, а выбранное проклятие **нельзя изменить**.
- Применение проклятия проходит через **подтверждение** (необратимо).

**Технически в State:**
- `GameState.curse`: активное проклятие (`null`, если не выбрано).
- `GameState.hasActed`: флаг, что игрок уже совершил действие (блокирует выбор).

**Проклятья:**
- **☁️ Туман (fog)**:
  - В начале каждого раунда скрывает **2 случайные карты** на столе.
  - Пока карты скрыты, их **нельзя трогать**: нельзя кликать/перетаскивать/целью выбирать/применять на них эффекты.
  - Когда на столе остаётся ≤2 карт — скрытые карты раскрываются.
- **🌕 Полнолуние (full_moon)**:
  - При смерти одного монстра лечит **всех остальных монстров** на +1 HP, но **не выше maxHP**.
  - Срабатывает независимо от причины смерти (оружие/магия/прочее), если смерть проходит через общий kill-путь.
- **🥦 Отравление (poison)**:
  - **Только зелья** (`potion`): лечат на **-1 HP** меньше.
  - Стакается с *Гнилью (Rot)*: итоговый модификатор может быть **-3** (редко).
- **🛠️ Закалка (tempering)**:
  - Все оружия (`weapon`) наносят **+1 урон**.
  - Влияет на *Зеркало (Mirror)*: “максимальная сила оружия” считается с учётом +1.
- **💰 Жадность (greed)**:
  - При использовании **любой монеты** (`coin`) игрок дополнительно получает **+2 💎**.
  - Это “облачный бонус”: **номинал монеты не меняется**.
  - Не усиливает *Подношение (Offering)*: лечение монстров идёт от номинала монеты (без +2).
- **🌑 Тьма (darkness)**:
  - Визуальный модификатор: экран темнеет, вокруг **курсора мыши** — круг света (как пасхалка Lumos).
  - Работает в забеге и в декбилдере.
  - Пока активна Тьма, пасхалка **Lumos/Nox отключена**.

### Взятие карт (`TAKE_CARD_TO_HAND`)
*   Игрок может взять карту со стола в левую руку, правую руку или рюкзак, если слот свободен.
*   **Зелья:** Если берутся в левую/правую руку — выпиваются мгновенно. Если в рюкзак — сохраняются.
    *   *Гниль (Rot):* Если активен дебафф, лечение снижается на 2.
    *   *Отравление (Poison):* Если активно проклятие, лечение снижается на 1 (только для зелий).
    *   *Закуска (Snack):* Если активно, избыточное лечение (Overheal) превращается в монеты.
*   **Монеты:** Собираются мгновенно при попытке взять в руку (превращаются в валюту).
    *   *Подношение (Offering):* Если на столе есть монстр с этой способностью, монета не дается игроку, а лечит монстра.
    *   *Жадность (Greed):* Дополнительно +2 💎 **игроку** (номинал монеты не меняется; лечение по Подношению не усиливает).
*   **Ограничения:**
    *   *Паутина (Web):* Блокирует использование рюкзака.
    *   *Эхо (Echo):* Дублирует взятый предмет в рюкзак (если есть место).

### Бой (`INTERACT_WITH_MONSTER`)

## 9. Dev/Debug: скрытые функции InfoHUD (читы)

В игре есть **скрытые dev‑кнопки**, доступные из InfoHUD:
- UI: `src/components/game/hud/GameBottomBar.tsx`
- Прокидывание обработчиков: `src/components/game/screens/useGameScreenController.ts`
- Механика (source of truth): `src/features/game/domain/reducer/gameReducer.ts`

### Как открыть
Нижняя панель → **Info** → нажать **глаз** (“Показать читы”) → появляется блок с кнопками.

### Доступные действия
- **Добавить 💎 герою (run coins)**:
  - спрашивает число через prompt и диспатчит `CHEAT_ADD_COINS`.
  - изменяет `GameState.player.coins` (это валюта забега, используется торговцем).
- **Запланировать торговца на следующий раунд**:
  - диспатчит `CHEAT_SCHEDULE_MERCHANT_NEXT_ROUND`.
  - выставляет `merchant.willAppear=true`, `merchant.scheduledRound = currentRound + 1`,
    `merchant.hasAppeared=false`, `merchant.forceOpenNextRound=true`.
  - `forceOpenNextRound` — читовый флаг: позволяет открыть торговца в следующем раунде
    **без условия “ровно 1 карта на столе”**; сбрасывается при открытии торговца.

#### Атака Монстра Оружием
*   Оружие одноразовое. После удара оно уходит в сброс.
*   **Урон:** Если Урон >= HP Монстра, монстр умирает. Иначе HP монстра уменьшается.
*   **Overkill:** Избыточный урон записывается в статистику.
*   *Бегство (Flee):* Если у монстра остается 3 HP или меньше, он может сбежать обратно в колоду.

#### Атака Монстром (Защита)
Игрок может "перетащить" монстра на себя или свой щит.
1.  **Удар по телу (Player):**
    *   Игрок получает урон, равный атаке монстра.
    *   Монстр **умирает** после атаки (самопожертвование), если не сработал *Отвод (Deflection)*.
    *   *Доспехи (Armor):* Полностью блокируют один удар.
    *   *Отвод (Deflection):* Отражает урон в другого монстра или в самого атакующего.
2.  **Удар по щиту (Shield):**
    *   Щит поглощает урон. Прочность щита уменьшается.
    *   Если Урон > Прочности щита, остаток (Overflow) наносится игроку, щит ломается.
    *   Монстр умирает после атаки.
    *   *Топот (Trample):* Монстр с этой способностью игнорирует прочность и мгновенно ломает щит, но урон игроку не наносится (щит принимает удар ценой жизни).

### Использование Заклинаний (`USE_SPELL_ON_TARGET`)
Заклинания применяются из рук на цель (Игрок, Монстр, Карта).
*   **Silence (Молчание):** Блокирует использование любой магии, пока жив монстр с этой способностью.
*   **Список эффектов (примеры):**
    *   *Escape (Побег):* Замешивает всех врагов на столе обратно в колоду.
    *   *Leech (Кровосос):* Высасывает жизнь из монстра (урон монстру = лечение игроку).
    *   *Potionify:* Превращает предмет в зелье.
    *   *Sacrifice (Жертва):* Наносит монстру урон, равный потерянному HP игрока.
    *   *Merchant (Скупщик):* Удваивает стоимость предмета при продаже.
    *   *Split (Расщепление):* Делит монстра на двух с половиной HP.

### Продажа (`SELL_ITEM`)
*   Можно продать предмет из руки или со стола за монеты (значение карты).
*   Монстров продавать нельзя.
*   *Крик (Scream):* Блокирует возможность продажи.

### Сброс (`RESET_HAND`)
*   Позволяет сбросить текущие карты со стола в колоду и перераздать руку.
*   **Цена:** 5 HP.
*   **Условие:** Нельзя использовать, если HP <= 5 или на столе 4 карты (полная рука).

### Странствующий торговец (Traveling Merchant)
**Странствующий торговец** — это **междураундовое событие** (магазин), которое может появиться в забеге.

**Планирование появления (на старте забега):**
- В начале забега бросаем шанс: **40% торговец будет**, **60% — нет**.
- Если торговец будет, выбираем `scheduledRound` (раунд появления).
- Торговец не может появиться на 1-м раунде.

**Когда реально появляется:**
- Только **между раундами**, в момент перехода раунда (когда на столе осталось ≤1 карты).
- Фактически показывается **в начале раунда N (после `round++`)**.
- Торговец появляется **только если на столе была ровно 1 карта** (если 0 — не появляется).
- `scheduledRound` трактуется как **минимальный** раунд появления: если в запланированный переход на столе было 0 карт,
  торговец появится при **первом следующем** переходе раунда, где на столе осталась ровно 1 карта.

**Как выглядит (Variant B: overlay):**
- “Последняя карта” прошлого стола остаётся в своём слоте, но становится **перекрыта** токеном **🚪 “Уйти”** и недоступна для взаимодействий.
- В трёх остальных слотах показываются **3 товара** (артефакта).

**Разрешённые действия во время торговца:**
- Разрешено:
  - **🚪 “Уйти”** (drop на героя или в пустой слот инвентаря) — закрывает торговца и запускает обычный добор 3 карт из деки.
  - **покупка** товара (drag товара в **пустой** слот инвентаря) — списывает **15💎**, кладёт артефакт в выбранный слот, **сразу закрывает магазин** и запускает обычный добор 3 карт из деки.
  - **ровно 1 продажа предмета из рюкзака** (строго из `backpack`).
  - **системные действия**: “Новая игра” (перезапуск забега) и инициализация состояния должны работать даже при активном торговце.
- Запрещено:
  - бой/любые боевые взаимодействия,
  - каст спеллов,
  - сброс `RESET_HAND`,
  - прочие действия с картами на столе.

**Ограничения покупки:**
- Купить можно только если хватает **15💎**.
- Купить можно только в **свободный слот** (лев/прав/рюкзак) и он не должен быть заблокирован.
- Если свободен только рюкзак, но он заблокирован (например `web`) — купить нельзя, можно только “уйти”.

> Примечание по реализации: эффекты артефактов (🦁/🗡️/📜) внедряются следующей партией; сейчас реализованы покупка/уход/ограничения магазина.

### Артефакты торговца (после покупки)
Ниже — как ведут себя артефакты после покупки (они попадают в инвентарь как отдельные типы карт).

#### 🦁 Зелье Храбрости (`bravery_potion`)
- **Активация**: перетащить на героя (drop на `player`).
- **Эффект**: сначала **-4 HP**, затем **+2 maxHp**.
  - В **God Mode** самоурон игнорируется (maxHp всё равно увеличивается).
  - Может убить героя, если HP ≤ 4 (в обычном режиме).
- **После применения**: исчезает из слота и уходит в `discardPile` (как заклинание).
- **Продажа**: 0💎 (по сути “выкинуть”).

#### 🗡️ Клеймор (`claymore`)
- **Использование**: атакует как оружие (drag на монстра).
- **Урон**: равен текущей “прочности” (`value`) с учётом модификаторов оружия:
  - `tempering` (+1),
  - `miss` (-2 к следующей атаке),
  - `anvil` (+2 к урону оружия) — если уже применён к предмету.
- **Прочность**: после удара уменьшается на **HP монстра до удара**.
  - Если прочность падает до 0 — клеймор ломается и уходит в `discardPile`.
  - Иначе остаётся в руке с обновлённым `value`.
- **Mirror**: монстр со способностью `mirror` берёт силу по **текущей прочности** клеймора.
- **Продажа**: по текущей прочности (`value`).

#### 📜 Молитва (`prayer_spell`)
- **Активация**: перетащить “Молитву” на spell‑карту в руке (таргет).
- **Эффект**: создаёт копию выбранного спелла в **рюкзаке** (копируем `spellType`).
  - Если рюкзак занят или заблокирован `web` — эффект не срабатывает.
  - Разрешён кейс, когда сама “Молитва” лежит в рюкзаке: она “съедается”, а её слот становится копией.
- **После применения**: “Молитва” уходит в `discardPile`.
- **Продажа**: нельзя.

## 4. Способности Монстров (Abilities)

Способности делятся на срабатывающие при появлении (Spawn), при смерти (Kill), пассивные и активные при взаимодействии.

Полный список способностей, их иконок и детальных описаний эффектов находится в документе **[Каталог Игрового Контента](gameContent.md#2-способности-монстров-monster-abilities)**.

В данном документе (Mechanics) важно понимать их классификацию по триггерам (когда они срабатывают):

*   **Spawn (При появлении):** Ambush, Corpseeater, Mirror.
*   **Passive (Пассивные):** Silence, Stealth, Web, Scream, Rot, Exhaustion, Parasite.
*   **Kill (При смерти):** Commission, Whisper, Breach, Disarm, Blessing, Legacy, Beacon, Bones, Junk, Theft, Corrosion, Miss.
*   **Trigger/Combat (В бою/При условии):** Offering, Trample, Flee.

## 5. Техническая Реализация

### State Management
Используется паттерн `Reducer` (`gameReducer.ts`). Состояние (`GameState`) неизменяемо, каждое действие возвращает новый объект состояния.

**Ключевые поля State:**
*   `deck`: Массив карт (стек).
*   `enemySlots`: Массив из 4 элементов (`Card | null`).
*   `activeEffects`: Массив строк (`['miss', 'armor', ...]`) для временных баффов/дебаффов.
*   `hpUpdates`: Лог изменений здоровья для анимаций.
*   `logs`: Текстовый лог событий.

### Game Logic
*   **Очередность:** Обработка действий происходит синхронно.
*   **Логгер:** Все значимые действия пишутся в `logs` с типом (combat, heal, info).
*   **God Mode:** Флаг `isGodMode` предотвращает получение урона и позволяет делать сброс бесплатно.

### Ограничения Реализации
*   **Стек эффектов:** Эффекты типа "следующий удар" (`activeEffects`) снимаются сразу после срабатывания.
*   **Взаимодействие:** Блокировки (Stealth, Silence) проверяются в начале кейса редьюсера. Если условие блокировки активно, действие отменяется (`return state` или `break`).

## Правила обновления
- Обновлять этот документ, если меняются:
  - правила боя/раундов/победы/поражения,
  - определения “что считается действием” (например, влияет на `hasActed`),
  - правила проклятий/абилок (как механика, не как описание/иконка),
  - семантика `GameAction` и важные поля `GameState` (инварианты).
- Не обновлять при изменении только текстов/иконок/цветов — это `gameContent.md`.

## Связанные документы
- `memory-bank/gameContent.md`
- `memory-bank/uiDesign.md`
- `memory-bank/systemPatterns.md`

## Последнее обновление
- 2025-12-19 — Приведено к формату Memory Bank (Блок 90): добавлены Source of Truth / Entry points / правила обновления / Last updated.

