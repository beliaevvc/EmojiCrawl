...
Дополнительно (текущий UX-полиш, paused):
- **DnD drag-preview**: введён кастомный `GameDragLayer`, чтобы исправить “уезд” preview при `transform/scale` (особенно заметно на 🗡️ Клейморе/артефактах торговца).
- **Известная проблема (Chrome)**: отмена drag (“snapback” при отпускании в пустоте) пока **не 1:1** с прежним нативным HTML5 preview — по ощущениям пользователя есть “задержка/мигание”. Работу по полишу **временно остановили**.
...
- 2025-12-20 — InfoHUD: добавлены скрытые dev‑инструменты рядом с `God Mode`:
  - кнопка **+💎** (через модалку, без `prompt()`),
  - кнопка **“Торговец на следующий раунд”** (форс‑планирование).
  - Механика заведена доменными экшенами `CHEAT_ADD_COINS` и `CHEAT_SCHEDULE_MERCHANT_NEXT_ROUND`,
    UI подключён через `GameBottomBar` → `useGameScreenController` → `GameModals`.
