...
const DEFAULT_WEAPONS = [2, 3, 4, 5, 6, 7];
...
const WeaponsEditor = ({ initialValues, onSave, onClose }: WeaponsEditorProps) => {
    const [weapons, setWeapons] = useState<number[]>([...initialValues]);
    const [isDeleteMode, setIsDeleteMode] = useState(false);
    const [itemToDelete, setItemToDelete] = useState<number | null>(null);

    const handleChange = (index: number, newValue: number) => {
        const newWeapons = [...weapons];
        newWeapons[index] = newValue;
    ...
    const confirmDelete = () => {
        if (itemToDelete !== null) {
            setWeapons(weapons.filter((_, i) => i !== itemToDelete));
            setItemToDelete(null);
            if (weapons.length <= 1) setIsDeleteMode(false); 
        }
    };
            ...
            >
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-4">
                        <div>
                            <h2 className="text-2xl font-display font-bold text-stone-200 uppercase tracking-widest">Оружие</h2>
                            <p className="text-stone-500 text-xs mt-1">Настройка колоды оружия</p>
                        </div>
                        ...
                        <div className="flex flex-col justify-center">
                             ...
                             <span className="text-xl font-mono font-bold text-stone-300 leading-none">{weapons.length}</span>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 -mx-2">
                    ...
                    {weapons.length === 0 && (
                        <div className="text-center text-stone-600 py-10">Нет оружия в колоде</div>
                    )}
                </div>
                ...
                <div className="flex gap-4 w-full pt-6 border-t border-stone-800 mt-4">
                    <button 
                        onClick={() => setWeapons([...DEFAULT_WEAPONS])}
                ...
                <AnimatePresence>
                    {itemToDelete !== null && (
                        <ConfirmationModal 
                            ...
                            message="Вы уверены, что хотите удалить это оружие из колоды?"
