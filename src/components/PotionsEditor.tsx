...
interface PotionsEditorProps {
    initialValues: number[];
    onSave: (values: number[]) => void;
    onClose: () => void;
}

const DEFAULT_POTIONS = [2, 3, 4, 5, 6, 7, 8, 9, 10];

const PotionCardItem = ({ 
    value, 
    onChange, 
    onDelete 
}: { 
    ...
    <div className="flex flex-col items-center gap-2 relative group" style={{ margin: '10px' }}>
        <div className="relative">
            <div className="w-20 h-20 md:w-24 md:h-24 rounded-full border-4 border-emerald-500/50 bg-emerald-900/20 flex items-center justify-center text-3xl md:text-4xl shadow-lg relative overflow-visible">
                <span className="z-10 select-none">🧪</span>
                
                {/* Value Badge - Bottom Center */}
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 bg-stone-900 border-2 border-emerald-500 rounded-full w-10 h-10 flex items-center justify-center z-10 shadow-md">
...
const PotionsEditor = ({ initialValues, onSave, onClose }: PotionsEditorProps) => {
    const [potions, setPotions] = useState<number[]>([...initialValues]);
    const [itemToDelete, setItemToDelete] = useState<number | null>(null);

    const handleChange = (index: number, newValue: number) => {
        const newPotions = [...potions];
        newPotions[index] = newValue;
    ...
    const confirmDelete = () => {
        if (itemToDelete !== null) {
            setPotions(potions.filter((_, i) => i !== itemToDelete));
            ...
            >
                ...
                <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-4">
                        <div>
                            <h2 className="text-2xl font-display font-bold text-stone-200 uppercase tracking-widest">Зелья</h2>
                            <p className="text-stone-500 text-xs mt-1">Настройка колоды зелий</p>
                        </div>
                        ...
                        <div className="flex flex-col justify-center">
                             ...
                             <span className="text-xl font-mono font-bold text-stone-300 leading-none">{potions.length}</span>
                        </div>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-2 -mx-2">
                    ...
                    {potions.length === 0 && (
                        <div className="text-center text-stone-600 py-10">Нет зелий в колоде</div>
                    )}
                </div>
                ...
                <div className="flex gap-4 w-full pt-6 border-t border-stone-800 mt-4">
                    <button 
                        onClick={() => setPotions([...DEFAULT_POTIONS])}
                ...
                <AnimatePresence>
                    {itemToDelete !== null && (
                        <ConfirmationModal 
                            title="Удалить карту?"
                            message="Вы уверены, что хотите удалить это зелье из колоды?"
